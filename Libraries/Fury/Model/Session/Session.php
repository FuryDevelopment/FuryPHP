    ...
    class Session
    {
        ...
        public function EndSession()
        {
            ...
            if($this->SaveInDB())
                ...
                if($this->sessionExists())
                     $this->sql->query("DELETE FROM sessions WHERE 
        ...
        public function set_id()
        {
            ...
            if($this->SaveInDB())
            {
                ...
                //Do the query.
                $this->sql->query("INSERT INTO sessions(`sessionid`, `vars`) 
        ...
        public function get($var)
        {
            ...
            {
                //Set up the query:
                $q = $this->sql->query("SELECT * FROM " . $this->SessionTable . " 
                                      WHERE " . $this->SessionIDVar . " = 
                                      '" . $_SESSION[$this->SessionIDVar] . "'");
                //Do a number rows
                $n = $this->sql->num_rows($q);
                //If teh session exists
                if($n >= 1)
                {
                    //Setup the fetch array
                    $f = $this->sql->fetch_array($q);
        ...
        public function UpdateVars($data)
        {
            ...
            if($data != null)
            {
                ...
                $this->sql->query("UPDATE "  . $this->SessionTable . " SET 
                                 vars = '" . $data . "' 
                                 WHERE "   . $this->SessionIDVar . " = 
                                 '" . $_SESSION[$this->SessionIDVar] . "'");
            }
        ...
        public function SaveInDB()
        {
            ...
            if($this->saveindb && $this->dbsave && $this->sql->allowConnection)
                return true;
